...
ifeq ($(strip $(RUNTIME_ENV)),local)
	...
	terraform -chdir=terraform/${CLOUD} fmt --recursive
else ifeq ($(strip $(RUNTIME_ENV)),container)
	make restart
	docker exec -it ${CLOUD}-terraform-packer terraform -chdir=terraform/${CLOUD} init
endif
...
ifeq ($(strip $(RUNTIME_ENV)),local)
	...
	terraform -chdir=terraform/${CLOUD} plan -out=plan/tfplan.binary -var-file vars.tfvars
else ifeq ($(strip $(RUNTIME_ENV)),container)
	docker exec -it ${CLOUD}-terraform-packer terraform -chdir=terraform/${CLOUD} validate
	docker exec -it ${CLOUD}-terraform-packer terraform -chdir=terraform/${CLOUD} plan -out=plan/tfplan.binary -var-file vars.tfvars
endif

.PHONY: terra-sec
terra-sec: terra-plan			## Security Check Terraform
ifeq ($(strip $(RUNTIME_ENV)),local)
	terraform -chdir=terraform/${CLOUD} show -json plan/tfplan.binary > terraform/${CLOUD}/plan/tfplan.json
	checkov -f terraform/${CLOUD}/plan/tfplan.json
else ifeq ($(strip $(RUNTIME_ENV)),container)
	docker exec -it ${CLOUD}-terraform-packer terraform -chdir=terraform/${CLOUD} show -json plan/tfplan.binary > terraform/${CLOUD}/plan/tfplan.json
	docker exec -it ${CLOUD}-terraform-packer checkov -f terraform/${CLOUD}/plan/tfplan.json
endif

.PHONY: terra-lint
terra-lint: 				## Lint Terraform
ifeq ($(strip $(RUNTIME_ENV)),local)
	tflint terraform/${CLOUD}/ --init --config=terraform/${CLOUD}/.tflint.hcl --var-file=terraform/${CLOUD}/vars.tfvars
	tflint terraform/${CLOUD}/ --config=terraform/${CLOUD}/.tflint.hcl --var-file=terraform/${CLOUD}/vars.tfvars
else ifeq ($(strip $(RUNTIME_ENV)),container)
	make terra-init 
	docker exec -it ${CLOUD}-terraform-packer tflint terraform/${CLOUD}/ --init --config=terraform/${CLOUD}/.tflint.hcl --var-file=terraform/${CLOUD}/vars.tfvars
	docker exec -it ${CLOUD}-terraform-packer tflint terraform/${CLOUD}/ --config=terraform/${CLOUD}/.tflint.hcl --var-file=terraform/${CLOUD}/vars.tfvars
endif

.PHONY: terra-apply
terra-apply: terra-plan			## Apply Terraform
ifeq ($(strip $(RUNTIME_ENV)),local)
	terraform -chdir=terraform/${CLOUD} apply plan/tfplan.binary
else ifeq ($(strip $(RUNTIME_ENV)),container)
	docker exec -it ${CLOUD}-terraform-packer terraform -chdir=terraform/${CLOUD} apply plan/tfplan.binary
endif
...
ifeq ($(strip $(RUNTIME_ENV)),local)
	...
	docker exec -it ${CLOUD}-terraform-packer terraform -chdir=terraform/${CLOUD} output
endif

.PHONY: terra-destroy
terra-destroy: terra-init		## Destroy Terraform
ifeq ($(strip $(RUNTIME_ENV)),local)
	terraform -chdir=terraform/${CLOUD} destroy -var-file vars.tfvars -auto-approve
else ifeq ($(strip $(RUNTIME_ENV)),container)
	docker exec -it ${CLOUD}-terraform-packer terraform -chdir=terraform/${CLOUD} destroy -var-file vars.tfvars -auto-approve
endif
